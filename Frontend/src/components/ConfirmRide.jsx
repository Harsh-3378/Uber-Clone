    ...
    <div>
      <h5 className="p-3 text-center absolute top-0 w-full">
        <i
          ...
          onClick={() => props.setConfirmRideOpen((prev) => !prev)}
        ></i>
      </h5>
      ...
      <div className="flex gap-3 justify-between items-center flex-col">
        ...
        <button onClick={()=> {props.setConfirmRideOpen(prev => !prev); props.setVehicleFound(prev => !prev)}} className="w-full bg-green-600 text-white font-semibold p-2 rounded-lg mt-5">
          Confirm
        </button>
      </div>
    </div>
